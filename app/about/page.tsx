    ...
    <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8 py-10 sm:py-14 w-full">
      ...
      <section className="mb-12">
        ...
        <p className="mt-4 max-w-2xl text-bone-200/70 font-semibold text-base sm:text-lg">
          Arena 15 is made by @SANTI and @NCLBN — two developers who wanted a fast, no-download 1v1 game platform they could actually use with friends.
        </p>

        <div className="mt-6 grid grid-cols-1 sm:grid-cols-2 gap-4 max-w-xl">
          {DEVS.map((d) => (
            <div key={d.handle} className="relative border-[3px] border-black rounded-chunk bg-ink-800 p-6 shadow-pop-lg overflow-hidden">
              <div className={`absolute -top-6 -right-6 w-24 h-24 ${d.accent} rounded-full border-[3px] border-black opacity-40`} />
              <div className={`w-14 h-14 ${d.accent} border-[3px] border-black rounded-chunk flex items-center justify-center shadow-pop mb-4`}>
                <span className="font-display text-xl text-black">@</span>
              </div>
              <div className="font-display text-2xl text-bone-50">@{d.handle}</div>
              <div className="text-xs text-bone-200/60 font-semibold mt-1 uppercase tracking-widest">{d.role}</div>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="mb-12">
        ...
        <h2 className="font-display text-3xl text-bone-50">Built for chaos.<br /><span className="text-lemon">Shipped on the edge.</span></h2>
        <p className="mt-4 max-w-2xl text-bone-200/70 font-semibold text-base sm:text-lg">
